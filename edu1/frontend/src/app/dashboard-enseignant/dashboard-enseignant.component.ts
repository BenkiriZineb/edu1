...
export class DashboardEnseignantComponent implements OnInit {
  ...
  chargerDonnees(): void {
    ...
    this.educationService.getNotes(0).subscribe(notes => {
      this.notes = notes.filter(n => this.enseignant?.matieres.includes(this.getMatiereNom(n.matiereId)));
    });
  ...
  // Gestion des notes
  ajouterNote(): void {
    if (this.nouvelleNote.eleveId && this.nouvelleNote.matiereId && this.nouvelleNote.valeur && this.nouvelleNote.noteMaximale) {
      const note: Note = {
        id: 0,
        eleveId: this.nouvelleNote.eleveId!,
        matiereId: this.nouvelleNote.matiereId!,
        valeur: this.nouvelleNote.valeur!,
        noteMaximale: this.nouvelleNote.noteMaximale!,
  ...
  getMoyenneClasse(matiere: string): number {
    const notesMatiere = this.notes.filter(n => n.matiereId === this.getMatiereId(matiere));
  ...
  getNotesByMatiere(matiere: string): Note[] {
    return this.notes.filter(n => n.matiereId === this.getMatiereId(matiere));
  }

  getNotesCountByMatiere(matiere: string): number {
    return this.notes.filter(n => n.matiereId === this.getMatiereId(matiere)).length;
  }

  // Méthode helper pour obtenir l'ID d'une matière par son nom
  getMatiereId(matiereNom: string): number | undefined {
    const matiere = this.matieres.find(m => m.nom === matiereNom);
    return matiere?.id;
  }

  // Méthode helper pour obtenir le nom d'une matière par son ID
